...
//           All rights reserved
// 
//           filename : AuditLogAppService.cs
//           description :
// 
...
using Magicodes.Admin.Application.Auditing.Exporting;
...
using EntityHistoryHelper = Magicodes.Admin.Core.EntityHistory.EntityHistoryHelper;
        ...
        public List<NameValueDto> GetEntityHistoryObjectTypes()
        {
            ...
            if (AbpSession.TenantId == null)
                entityHistoryConfig = entityHistoryConfig
                    .Where(c => EntityHistoryHelper.HostSideTrackedTypes.Select(x => x.FullName)
                        .Contains(c.Value.ToString()))
                    .ToDictionary(key => key.Key, value => value.Value);
            else
                entityHistoryConfig = entityHistoryConfig
                    .Where(c => EntityHistoryHelper.TenantSideTrackedTypes.Select(x => x.FullName)
                        .Contains(c.Value.ToString()))
                    .ToDictionary(key => key.Key, value => value.Value);
