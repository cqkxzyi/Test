...
//           All rights reserved
// 
//           filename : TenantSettingsAppService.cs
//           description :
// 
...
using Magicodes.Admin.Application.Configuration.Tenants.Dto;
...
namespace Magicodes.Admin.Application.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        public async Task<TenantSettingsEditDto> GetAllSettings()
        {
            ...
            if (!_multiTenancyConfig.IsEnabled) settings.Email = await GetEmailSettingsAsync();
        ...
        private async Task<TenantUserManagementSettingsEditDto> GetUserManagementSettingsAsync()
        {
            return new TenantUserManagementSettingsEditDto
            {
                ...
                UseCaptchaOnRegistration =
                    await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.UseCaptchaOnRegistration)
            };
        }
        ...
        private async Task UpdateBillingSettingsAsync(TenantBillingSettingsEditDto input)
        {
            ...
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
                AppSettings.TenantManagement.BillingTaxNumber, input.TaxNumber);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
                AppSettings.TenantManagement.BillingContact, input.Contact);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
                AppSettings.TenantManagement.BillingBank, input.Bank);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
                AppSettings.TenantManagement.BillingBankAccount, input.BankAccount);
        }
