...
using Magicodes.Admin.Core.MultiTenancy.Demo;
...
namespace Magicodes.Admin.Core.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(LocalizationConsts.LocalizationSourceName)
        ...
        public async Task<SubscriptionPayment> GetLastPaymentAsync(
            Expression<Func<SubscriptionPayment, bool>> predicate)
        {
            ...
            int remainingDaysCount)
        {
            ...
            if (remainingDaysCount > (int) PaymentPeriodType.Annual)
            {
                ...
                additionalPrice +=
                    GetAnnualCalculatedPrice(currentEdition, targetEdition,
                        remainingsYearsCount); // add yearly price to montly calculated price
            }
            else
            {
                additionalPrice = GetMonthlyCalculatedPrice(currentEdition, targetEdition, remainingDaysCount);
        ...
        private static decimal GetMonthlyCalculatedPrice(SubscribableEdition currentEdition,
            SubscribableEdition upgradeEdition, int remainingDaysCount)
        {
            decimal currentUnusedPrice = 0;
            decimal upgradeUnusedPrice = 0;

            if (currentEdition.MonthlyPrice.HasValue)
                currentUnusedPrice = currentEdition.MonthlyPrice.Value / (int) PaymentPeriodType.Monthly *
                                     remainingDaysCount;

            if (upgradeEdition.MonthlyPrice.HasValue)
                upgradeUnusedPrice = upgradeEdition.MonthlyPrice.Value / (int) PaymentPeriodType.Monthly *
                                     remainingDaysCount;
        ...
        private static decimal GetAnnualCalculatedPrice(SubscribableEdition currentEdition,
            SubscribableEdition upgradeEdition, int remainingYearsCount)
        {
