...
using Magicodes.Admin.Application.Authorization.Users.Profile.Cache;
using Magicodes.Admin.Application.Authorization.Users.Profile.Dto;
using Magicodes.Admin.Application.Gdpr;
...
namespace Magicodes.Admin.Application.Authorization.Users.Profile
{
    [AbpAuthorize]
    public class ProfileAppService : AdminAppServiceBase, IProfileAppService
    {
        ...
        {
            var user = await UserManager.GetUserByIdAsync(AbpSession.GetUserId());
            if (user.ProfilePictureId == null) return new GetProfilePictureOutput(string.Empty);
        ...
        public async Task<GetProfilePictureOutput> GetFriendProfilePictureById(GetFriendProfilePictureByIdInput input)
        {
            if (!input.ProfilePictureId.HasValue ||
                await _friendshipManager.GetFriendshipOrNullAsync(AbpSession.ToUserIdentifier(),
                    new UserIdentifier(input.TenantId, input.UserId)) ==
                null) return new GetProfilePictureOutput(string.Empty);
        ...
        {
            await _binaryObjectManager.DeleteAsync(profilePictureId);
        }
    }
}